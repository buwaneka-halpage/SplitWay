...
**Goal:** Restyle the existing one-page flow with shadcn/ui, keep it a usable iOS/Android PWA, keep e2e green (including Pixel 5), put the live URL in the README. Engine math and `data-testid`s stay.
...
- Restyle People → Expenses → Balances → Settle on **one page**. Mobile: ~44px tap targets, safe-area padding, bottom section nav (`#people` `#expenses` `#balances` `#settle`).
...
**Do not:** change `src/lib/money|splits|balances|settle|store|types.ts` public API; add auth/routes/percentage split; commit `.env` / `.vercel`; merge the PR (mastermind merges).
...
**Hard product locks:** no auth; LKR cents; equal + exact only; localStorage `splitway:v1`; never name other expense-splitter products; never commit secrets.
